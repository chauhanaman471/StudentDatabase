/**
 * @file StudentDb.cpp
 * @brief Implementation of the StudentDb class to manage students and courses.
 * @author Aman Chauhan
 * @date 17 Jan 2025
 * @version 1.0
 * @note Master Student at HDA
 *
 * This file contains the implementation of the methods related to adding,
 * listing, and managing students, courses, and enrollments in the student
 * database. It also includes file I/O methods for reading and writing data.
 */
...
void StudentDb::addCourse(unsigned int courseKey, std::string title,
	...
	case 'W':{
	    ...
	    //Create WeeklyCourse object with corresponding day using a switch-case
...
void StudentDb::addEnrollment(int matrikelNumber, int courseKey,
	...
	if(students.find(matrikelNumber)==students.end()){
		cout<<"Student with matrikelNumber: "<<matrikelNumber
				<<" doesn't exist"<<endl;
		return;
	}
	...
	//get reference to associated student object stored in map
...
void StudentDb::updateStudent(int matrikelNumber) {
	//iterate over map to first find student Matrikel Number and
	//its object stored in DB
	for(auto & pair : students){
		...
		if (studentMatrikelNumber == matrikelNumber){
			cout <<"Student with Matrikel Number: "<<matrikelNumber
					<<" exist"<<endl;
			...
			while(updateChoice !=0){
                ...
                if (cin.fail()) {
                    ...
                    cout << "Invalid input: "
                    		"Please enter a number between 0 and 8." << endl;
				...
				case 3:{
					...
					(updatedYear,updatedMonth,updatedDate);

					myStudent.setDateofBirth(updateDOB);
					break;
				}
				case 4:{
					cout <<"Enter updated Student Address:Street Name"<<endl;
					cin.ignore();
				...
				case 5:{
					cout <<"Enter updated Student Address: Postal Code"<<endl;
				...
				case 6:{
					cout <<"Enter updated Student Address: City Name"<<endl;
					cin.ignore();
				...
				case 8:{
					...
					cout <<"Enter the choice(1/2 : "
							"Delete enrollment/Update marks)"<<endl;
					...
					if(enrollmentChoice ==1){
						cout<<"User decided to delete "
								"the enrollment of student"<<endl;
						// Iterate into studentEnrollment vector to
						...
						cout << "User decided to update the marks of student"
								<< endl;
						// Loop into studentEnrollment vector
						    for (auto& enrollmentItr : studentEnrollment) {
						        // Using enrollment object to
						    	// find the course ptr and access course Key
						        // Check if course key matches
						        if (enrollmentItr.getCourse()->getCourseKey()
						        		== courseKey) {
						            cout << "Course found with courseKey: "
						            		<< courseKey << endl;
						            cout << "Enter Grade for the course: "
						            		<< endl;
						            float studentGrade;
						            cin >> studentGrade;
						            // Directly modifying the original object
						            enrollmentItr.setGrade(studentGrade);
...
void StudentDb::write(std::ostream &out) {
	...
	out<<courses.size();
	out<<"\n";
	//for each item(course object) stored in courses map call write function
	...
	out<<students.size();
	out<<"\n";
	//for each item(student object) stored in students map call write function
...
void StudentDb::read(std::istream &in) {
	...
	for(int enrollmentItr=0; enrollmentItr<enrollmentNumber;enrollmentItr++){
	    ...
	    auto studentIt = students.find(matrikelNumber);

	    // Find the course in the map
	    auto courseIt = courses.find(courseKey);

	    // Get raw pointer to the course
	    const Course* coursePtr = courseIt->second.get();
	    // Create Enrollment object
	    Enrollment enrollment(semester, grades, coursePtr);
	    // Add enrollment to the student's enrollment vector
	    studentIt->second.getEnrollments().push_back(enrollment);
